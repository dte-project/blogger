...
function blogger(text) {
    ...
    text = text.replace(/ class="separator align-center"/g, ' class="container center"');
...
var textarea = d.querySelector('#postingHtmlBox');
textarea.value = paragraph(blogger(validate(textarea.value)));

var div = d.createElement('div'), c;
div.innerHTML = textarea.value;
while (c = div.firstChild) {
    if (c.nodeType === 3 && !/^\s+$/.test(c.nodeValue)) {
        alert('Still not possible to add paragraph tags. Try adding some line break after </div> tag.');
        return;
    }
    div.removeChild(c);
}
