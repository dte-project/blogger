...
(function(win, doc) {
    ...
    content.innerHTML = strip(source.innerHTML);
    ...
    function set() {
        ...
        if (i === 0) {
            page_set(cover);
            pager_set('<p><span><a href="#' + H.replace('%i%', 1) + '">' + text.enter + '</a></span></p>');
    ...
    win['_' + (hash + 1)] = function($) {
        ...
        for (i = 0; i < ad; ++i) {
            ...
            s += '<p>' + j.summary.$t.replace(/<.*?>/g, "").replace(/[<>]/g, "").slice(0, 400) + '&hellip;</p>';
            insert(ul, el('li', s));
        }
